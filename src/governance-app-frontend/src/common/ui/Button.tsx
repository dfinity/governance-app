...
export interface ButtonProps extends CommonProps, Omit<DetailedHTMLProps<ButtonHTMLAttributes<HTMLButtonElement>, HTMLButtonElement>, "color" | "ref"> {
    to?: never;
}

// Loosen Router Link props to avoid strict type mismatch with 'to' union string
interface LinkButtonProps extends CommonProps, Omit<RouterLinkProps, "children" | "color" | "size" | "style" | "to"> {
    to: string | any;
}
...
export const Button = ({
    ...
    const getVariantAndClasses = (): { variant: ShadcnButtonProps["variant"]; className?: string } => {
        switch (color) {
            case "primary":
                return { variant: "default" };
            case "secondary":
                // Use semantic secondary colors
                return { variant: "outline", className: "bg-background text-foreground hover:bg-accent hover:text-accent-foreground border-input" };
            case "tertiary":
                // Use semantic ghost colors
                return { variant: "ghost", className: "text-muted-foreground hover:text-foreground hover:bg-accent" };
            case "link-gray":
                return { variant: "link", className: "text-muted-foreground hover:text-foreground p-0 h-auto" };
            case "link-color":
                return { variant: "link", className: "text-primary hover:text-primary/80 p-0 h-auto" };
            case "primary-destructive":
                return { variant: "destructive" };
            case "secondary-destructive":
                return { variant: "outline", className: "border-destructive/50 text-destructive hover:bg-destructive/10 hover:text-destructive hover:border-destructive" };
            case "tertiary-destructive":
                return { variant: "ghost", className: "text-destructive hover:bg-destructive/10 hover:text-destructive" };
            case "link-destructive":
                return { variant: "link", className: "text-destructive hover:text-destructive/80 p-0 h-auto" };
            default:
                return { variant: "default" };
        }
    };
    ...
    const content = (
        <>
            {isLoading && (
                <Loader2 className={cn("mr-2 size-4 animate-spin", showTextWhileLoading ? "" : "absolute")} />
            )}
            ...
            <span className={isLoading && !showTextWhileLoading ? "invisible" : ""}>
                {children}
            </span>
