    ...
    return (
        <div className={cn("grid w-full gap-1.5", className)}>
            {label && (
                <Label
                    htmlFor={generatedId}
                    className={cn(isInvalid && "text-destructive", effectiveDisabled && "opacity-70 cursor-not-allowed")}
                >
                    {label}
                    {!hideRequiredIndicator && effectiveRequired && <span className="text-destructive ml-1">*</span>}
                </Label>
            )}

            <div className={cn("relative", wrapperClassName)}>
                {Icon && (
                    <div className={cn("absolute left-2.5 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none", iconClassName)}>
                ...
                <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-2">
                    {shortcut && !isInvalid && !tooltip && (
                        <div className="pointer-events-none flex items-center">
                            <span className="text-xs text-muted-foreground border rounded px-1.5 py-0.5 bg-muted">
                        ...
                        <TooltipProvider>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <HelpCircle className={cn("size-4 text-muted-foreground cursor-pointer hover:text-foreground", tooltipClassName)} />
                                </TooltipTrigger>
                                <TooltipContent>
                                    <p>{tooltip}</p>
                                </TooltipContent>
