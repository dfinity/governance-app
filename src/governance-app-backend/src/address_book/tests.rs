...
fn set_and_get_icp_address() {
    ...
    assert_eq!(set_address_book(caller, book.clone()), SetAddressBookResponse::Ok);
...
fn set_replaces_entire_address_book() {
    ...
    assert_eq!(set_address_book(caller, book1.clone()), SetAddressBookResponse::Ok);
    ...
    assert_eq!(set_address_book(caller, book2.clone()), SetAddressBookResponse::Ok);
    assert_eq!(get_address_book(caller), GetAddressBookResponse::Ok(book2));

    let book3 = make_address_book(vec![("Charlie", icp(TEST_ICP_ADDRESS)), ("Dave", icrc1(TEST_ICRC1_ADDRESS)), ("Eve", icrc1(TEST_ICRC1_ADDRESS_WITH_SUBACCOUNT))]);
    assert_eq!(set_address_book(caller, book3.clone()), SetAddressBookResponse::Ok);
...
fn rejects_duplicate_names() {
    ...
    assert_eq!(
        set_address_book(caller, book),
        SetAddressBookResponse::DuplicateAddressName { name: "Alice".to_string() }
    );
}
...
fn failed_set_does_not_modify_data() {
    ...
    assert_eq!(set_address_book(caller, good_book.clone()), SetAddressBookResponse::Ok);
    ...
    assert_eq!(get_address_book(caller), GetAddressBookResponse::Ok(good_book));
}
